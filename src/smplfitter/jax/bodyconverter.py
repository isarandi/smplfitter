"""Body model parameter converter (JAX implementation)."""
...
class BodyConverter:
    ...
    def __init__(
        self,
        body_model_in: 'smplfitter.jax.BodyModel',
        body_model_out: 'smplfitter.jax.BodyModel',
    ):
    ...
    def convert(
        ...
        known_output_pose_rotvecs: Optional[jnp.ndarray] = None,
        known_output_shape_betas: Optional[jnp.ndarray] = None,
        known_output_kid_factor: Optional[jnp.ndarray] = None,
        num_iter: int = 1,
    ...
    def convert_vertices(self, inp_vertices: jnp.ndarray) -> jnp.ndarray:
        """
        ...
        # Convert to numpy for sparse matmul, then back to JAX
        inp_np = np.asarray(inp_vertices)
        v = inp_np.transpose(1, 0, 2).reshape(self.body_model_in.num_vertices, -1)
        r = self.vertex_converter_csr @ v
        out_np = r.reshape(self.body_model_out.num_vertices, batch_size, 3).transpose(1, 0, 2)
