    ...
    if weighted:
        ...
        def fit_fn(verts, joints, vertex_weights, joint_weights):
            res = fitter.fit(
                ...
                beta_regularizer2=l2_regularizer2,
        ...
        def wrapped(verts, joints, vertex_weights, joint_weights):
            if isinstance(verts, tf.RaggedTensor):
                res = wrapped(
                    verts.flat_values,
                    joints.flat_values,
                    vertex_weights.flat_values,
                    joint_weights.flat_values,
                )
                return tf.nest.map_structure(
                    lambda x: tf.RaggedTensor.from_nested_row_splits(x, verts.nested_row_splits),
                    res,
                )
            else:
        ...
        def wrapped(verts, joints):  # type: ignore[misc]
            if isinstance(verts, tf.RaggedTensor):
