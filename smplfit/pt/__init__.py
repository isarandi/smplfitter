...
def get_cached_fit_fn(
        ...
        beta_regularizer=1.0, beta_regularizer2=0.0, num_iter=3, vertex_subset=None, joint_regressor=None,
    ...
    with torch.device(device):
        body_model = SMPL(gender=gender, model_name=body_model_name)
        fitter = smplfit.pt.fitting.SMPLfit(
        ...
        def wrapped(verts, joints, vertex_weights, joint_weights):
            ...
            vertex_weights_resh = vertex_weights.view(-1, fitter.num_vertices) if vertex_weights is not None else None
            joint_weights_resh = joint_weights.view(-1, body_model.num_joints) if joint_weights is not None else None
...
def fit(verts: torch.Tensor, joints: Optional[torch.Tensor] = None,
        ...
        beta_regularizer2=0.0, num_iter=3, vertex_subset=None, joint_regressor=None, share_beta=False,
        final_adjust_rots=True, scale_target=False, scale_fit=False, scale_regularizer=0.0,
        kid_regularizer=None, device=None):
    fit_fn = get_cached_fit_fn(
        ...
        scale_target, scale_fit, scale_regularizer, kid_regularizer, device)
