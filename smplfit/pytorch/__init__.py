...
import os
import torch
from smplfit.pytorch.smpl import SMPL
...
def get_fit_fn(
        ...
        final_adjust_rots=True, weighted=False, scale_target=False, scale_fit=False,
        scale_regularizer=0, kid_regularizer=None):
    from smplfit.pytorch import fitting
    ...
    if weighted:
        ...
        def wrapped(verts, joints, vertex_weights, joint_weights):
            if isinstance(verts, torch.Tensor) and isinstance(joints, torch.Tensor):
                verts_resh = verts.view(-1, fitter.num_vertices, 3)
                joints_resh = joints.view(-1, body_model.num_joints, 3)
                vertex_weights_resh = vertex_weights.view(-1, fitter.num_vertices)
                joint_weights_resh = joint_weights.view(-1, body_model.num_joints)
